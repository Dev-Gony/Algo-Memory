...
assert.match(html, /cloud=neon|neon-shadow|ALGO_MEMORY_NEON/, 'built app must include Neon shadow provider');
assert.doesNotMatch(html, /postgres(?:ql)?:\/\/[^\s'"]+:[^\s@'"]+@/i, 'privileged Postgres URLs must never reach the browser');
